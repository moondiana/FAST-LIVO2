...
void LoopClosing::ComputeForCandidate(lightning::LoopCandidate& c) {
    if (c.idx1_ < 0 || c.idx2_ < 0 || static_cast<size_t>(c.idx1_) >= all_keyframes_.size() ||
        static_cast<size_t>(c.idx2_) >= all_keyframes_.size()) {
        c.ndt_score_ = 0;
        return;
    }

    const int submap_idx_range = 40;
    auto kf1 = all_keyframes_.at(c.idx1_);
    auto kf2 = all_keyframes_.at(c.idx2_);
    ...
    auto submap_kf1 = build_submap(static_cast<int>(kf1->GetID()), true);

    CloudPtr submap_kf2 = kf2->GetCloud();

    if (submap_kf1 == nullptr || submap_kf2 == nullptr || submap_kf1->empty() || submap_kf2->empty()) {
        c.ndt_score_ = 0;
        return;
    }
    ...
    /// 不同分辨率下的匹配（与 loop/src 一致：每轮新建 NDT，避免 VoxelGridCovariance 析构问题）
    ...
    for (auto& r : res) {
        ...
        rough_map1 = VoxelGrid(submap_kf1, static_cast<float>(r * 0.1));
        rough_map2 = VoxelGrid(submap_kf2, static_cast<float>(r * 0.1));
        if (rough_map1 == nullptr || rough_map2 == nullptr || rough_map1->empty() || rough_map2->empty()) {
            continue;
        }
...
void LoopClosing::PoseOptimization() {
    ...
    for (auto& c : candidates_) {
        ...
        if (v1 == nullptr || v2 == nullptr) {
            LOG(WARNING) << "skip loop edge because vertex missing: " << c.idx1_ << " -> " << c.idx2_;
            continue;
        }
